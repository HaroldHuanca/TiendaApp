...
# ✅ Insertar usuario
def insertar_usuario(nombre_usuario: str, contrasena: str, correo: str, descripcion_estado: str) -> Optional[int]:
    with DatabaseManager() as db:
...
def actualizar_mac(nombre_usuario: str) -> None:
    with DatabaseManager() as db:
        db.execute(
            text("CALL proc_actualizar_mac(:p_nombre_usuario)"),
            {"p_nombre_usuario": nombre_usuario}
        )
        db.commit()